...
export default function RegisterPage() {
  ...
  const [formData, setFormData] = useState({ name: '', email: '', phone: '', password: '', confirmPassword: '', universityId: '', gender: 'male' });
    ...
    const fetchUnis = async () => {
      try {
        ...
        if (res.universities && res.universities.length > 0) {
          ...
          setFormData(prev => ({ ...prev, universityId: res.universities[0].id }));
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!formData.name || !formData.email || !formData.phone || !formData.password || !formData.universityId || !formData.gender) {
    ...
    try {
      await register(firstName, lastName, formData.name, formData.email, formData.phone, formData.universityId, formData.password, formData.gender);
      navigate('/dashboard');
    } catch (err: any) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input type="text" value={formData.name} onChange={e => updateField('name', e.target.value)} placeholder="John Mensah" required className="w-full pl-10 pr-4 py-3 bg-gray-50 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-amber-400 text-sm" />
              </div>
            </div>
